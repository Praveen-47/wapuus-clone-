...
describe("FinnERC721", function () {
  ...
  beforeEach(async function () {
    ...
    Token = await ethers.getContractFactory("Finn_ERC721");
  ...
  describe("Transactions", function () {
    ...
    it("Exceed per mint limit", async function () {
      ...
      //expect(await hardhatToken.connect(addr1).mintFinns(21, {value: price})).to.be.reverted;
    });
    ...
    it("Withdrawl by owner", async function () {
      ...
      await hardhatToken.connect(addr1).mintFinns(1, {value: price})
    ...
    it("Set baseURI", async function () {
      const price = await hardhatToken.finnPrice()
      await hardhatToken.mintFinns(1, {value: price})
      
      await hardhatToken.setBaseURI("https://api.web3wp.com/Finns/")
      expect(await hardhatToken.tokenURI(0)).to.equal(
        "https://api.web3wp.com/Finns/0"
      );
    });

    it("Change Finn Name", async function () {
      const price = await hardhatToken.finnPrice()
      await hardhatToken.mintFinns(1, {value: price})


      const renamePrice = await hardhatToken.finnRenamePrice()
      await hardhatToken.changeFinnName(0,"Bubba Gumper", {value: renamePrice})
...
describe("Finns ERC1155", function () {
  // Mocha has four functions that let you hook into the the test runner's
  // lifecyle. These are: `before`, `beforeEach`, `after`, `afterEach`.
  ...
  beforeEach(async function () {
    // Get the ContractFactory and Signers here.
    Token = await ethers.getContractFactory("Finns");
    [owner, addr1, addr2, ...addrs] = await ethers.getSigners();
  ...
  describe("Transactions", function () {
    ...
    it("balanceOfFinn() is accurate", async function () {
      const price = await hardhatToken.finnPrice();
      // Check balances.
      expect(await hardhatToken.balanceOfFinn(owner.address)).to.equal(0);
      
      await hardhatToken.mintFinns(10, {value: price.mul(10)})
      await hardhatToken.connect(addr1).mintFinns(5, {value: price.mul(5)})

      // Check balances.
      expect(await hardhatToken.balanceOfFinn(owner.address)).to.equal(10);
      expect(await hardhatToken.balanceOfFinn(addr1.address)).to.equal(5);
    });
    ...
    it("tokensOfOwner() is accurate", async function () {
      ...
      await hardhatToken.mintFinns(2, {value: price.mul(2)})
    ...
    it("Exceed per mint limit", async function () {
      const price = await hardhatToken.finnPrice()
      //const response = await hardhatToken.connect(addr1).mintFinns(21, {value: price.mul(21)})
    ...
    it("Change Finn Name", async function () {
      ...
      const renamePrice = await hardhatToken.finnRenamePrice()
      await hardhatToken.changeFinnName(0,"Bubba Gumper", {value: renamePrice})
      expect(await hardhatToken.viewFinnName(0)).to.equal(
        "Bubba Gumper"
      );
    });
